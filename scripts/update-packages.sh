#!/usr/bin/env sh
# Portable updater: runs paru -Syu or yay -Syu depending on config.json (aurHelper)
...
# Read aurHelper from config.json without external deps (jq optional)
read_aur_helper() {
...
if HELPER=$(read_aur_helper); then
  ...
  if command -v paru >/dev/null 2>&1; then
    ...
    printf 'Error: No AUR helper specified in config.json (aurHelper) and neither paru nor yay found in PATH.\n' >&2
    exit 1
  fi
fi
